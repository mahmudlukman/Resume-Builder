...
const TemplateOne = ({ resumeData, colorPalette, containerWidth }: { resumeData: ResumeData; colorPalette?: string[]; containerWidth: number }) => {
  const themeColors = ((colorPalette ?? []).length > 0 ? colorPalette : DEFAULT_THEME) as string[];
            ...
            >
              {resumeData?.profileInfo?.profilePreviewUrl ? (
                ...
                <div
                  ...
                  style={{ color: themeColors?.[4] ?? DEFAULT_THEME[4]}}
                >
                  <LuUser />
                </div>
              )}
              ...
              {resumeData?.education?.map((data, index) => (
                <EducationInfo
                  ...
                  duration={`${formatYearMonth(
                    data.startDate ?? null
                  )} - ${formatYearMonth(data.endDate ?? null)}`}
                />
              ))}
            </div>
